...
export function InternalManagementView() {
    ...
    const badgeMap: Record<string, { label: string; color: string }> = {
        'EMPLOYEE_REGISTERED': { label: 'Alta Empleado', color: 'bg-blue-100 text-blue-700' },
        'PASSWORD_CHANGED': { label: 'Contraseña Cambiada', color: 'bg-purple-100 text-purple-700' },
        'USER_REGISTERED': { label: 'Registro Usuario', color: 'bg-indigo-100 text-indigo-700' },
    };
    ...
    return (
        <ERPLayout title="Gestión Interna (RRHH & Ops)" subtitle="Recursos humanos, soporte al cliente y comunidad">
            ...
            <div className="border-b border-gray-200 mb-6">
                <div className="flex gap-6">
                    {['Recursos Humanos'].map((tab) => (
                        <button key={tab} className="pb-3 text-sm font-medium border-b-2 border-blue-600 text-blue-600">
                            {tab}
                        </button>
                    ))}
                </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                {[
                    { label: 'Total Personal', value: employees.length, icon: Users, color: 'text-blue-600 bg-blue-50', sub: 'Empleados registrados' },
            ...
            <div className="flex gap-3 mb-6">
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors shadow-sm"
                >
